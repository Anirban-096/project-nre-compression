...
def get_21cm_data_seed(seedvalue):
        ...
        savefile_data = os.path.join(data_dir, 'data21cm_seed'+str(seedvalue))        
        np.savez(savefile_data, datacube_21cm = datacube, cylindrical_pow_spec_binned_data = cylindrical_pow_spec_binned_data, kpar_bins = kpar_bins, kpar_edges = kpar_edges, kperp_bins = kperp_bins, kperp_edges = kperp_edges, kount = kount)   


############ MAIN #######################
seedlist=np.genfromtxt('mockdata_seed_list.txt',dtype=int) ## the first 50 seeds of musicseed_list.txt

for i, seed in enumerate(tqdm(seedlist, desc="Generating mock 21cm data")):
    get_21cm_data_seed(seed)
