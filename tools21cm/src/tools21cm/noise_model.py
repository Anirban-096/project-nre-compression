...
def noise_cube_coeval(noise_seed, ncells, z, depth_mhz=None, obs_time=1000, filename=None, boxsize=None, total_int_time=6., int_time=10., declination=-30., uv_map=None, N_ant=None, verbose=True, fft_wrap=False):
        """
        ...
        np.random.seed(noise_seed)  # Set the seed for reproducibility
        if verbose: print("Noise seed is set to "+str(noise_seed)+"...")
        ...
        if n_jobs<=1:
                ...
                for k,zi in enumerate(zs):
                        ...
                        if '{:.3f}'.format(zi) not in uvs.keys():
                                uv_map, N_ant  = get_uv_map(ncells, zi, filename=filename, total_int_time=total_int_time, int_time=int_time, boxsize=boxsize, declination=declination, verbose=verbose)
