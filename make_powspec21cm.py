...
def get_21cm_powspec_seed(seedvalue):
        ...
        cylindrical_pow_spec_binned_sig, kount = ionization_map.get_binned_powspec_cylindrical(Delta_Tb_arr, kpar_edges, kperp_edges, convolve='True', units='') 
        ## units are mK^2 h^-3 cMpc^3 
                
        savefile = os.path.join(data_dir, 'sim21cm_seed'+str(seedvalue))        
        np.savez(savefile, signal_21cm = Delta_Tb_arr, cylindrical_pow_spec_binned_signal = cylindrical_pow_spec_binned_sig, kpar_bins = kpar_bins, kpar_edges = kpar_edges, kperp_bins = kperp_bins, kperp_edges = kperp_edges, kount = kount)   


############ MAIN #######################
seedlist=np.genfromtxt('musicseed_list.txt',dtype=int)

for i, seed in enumerate(tqdm(seedlist, desc="Running SCRIPT")):
    get_21cm_powspec_seed(seed)
